...
def time_scons(folder, procs):
    ...
    os.system('%s -o ../../results%s/%s/scons_cleanbuild.times lscons %s -j%s > /dev/null 2>&1' % (sconstest.time, procs, folder, sconstest.sconswd, procs))
    os.system('%s -o ../../results%s/%s/scons_update.times lscons %s -j%s > /dev/null 2>&1' % (sconstest.time, procs, folder, sconstest.sconswd, procs))
    os.system('%s -o ../../results%s/%s/scons_update_implicit.times lscons %s -j%s --max-drift=1 --implicit-deps-unchanged > /dev/null 2>&1' % (sconstest.time, procs, folder, sconstest.sconswd, procs))
    os.chdir(oldwd)
...
def prepare():
    ...
    if 'large' in timelist:
        os.system('python generate_libs.py large 200 100 15 5')
    if 'vlarge' in timelist:
        os.system('python generate_libs.py vlarge 300 100 15 5')
    if 'vvlarge' in timelist:
        os.system('python generate_libs.py vvlarge 400 100 15 5')
