...
def main():
    ...
    ptitle = "Clean builds (batchcompile)"
    ...
    if both:
        ...
        ca.info.color = 'green'
        ca.info.title = 'Parallel (-j4)'

        cb = sconstest.eval.DataCurve()
        cb.info.color = 'red'
        cb.info.title = 'Single'
    ...
    for i in range(len(ford)):
        if both:
            ...
            r, u, s = sconstest.eval.getTimeData(timefile(ford[i], builds[1], 
                                                 os.path.join('..','..','..','single_core','results')))
            if r != 0.0:
    ...
    ytitle = 'Time [s]'
    if both:
        sconstest.eval.plotDataCurves([ca, cb], 'compare_%s.png' % project, ptitle, 'C files', 
